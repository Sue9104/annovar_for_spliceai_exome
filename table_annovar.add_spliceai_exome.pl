...
our $REVISION = '$Revision: 887b78883bdfd32007295c84e416ef74caa01be0 $';
...
if ($vcfinput) {
	...
	$convertarg ||= '';
	$sc = "convert2annovar.pl $convertarg -includeinfo -allsample -withfreq -format vcf4 $queryfile > $tempfile.avinput";
...
sub printOrigOutput {
    ...
    ##MINSU: change output for one variant with multiple lines in db 
    ##my @oneline;
		##for my $i (0 .. @header-1) {
		##	my $item = $header[$i];
		##	my $expanded_field;
    ...
    ##if ($csvout) {
		...
		##}
    ## put spliceai_exome annotation to the last of header
    if ( grep (/spliceai_exome/, @header) ){
      @header = grep {$_ ne 'spliceai_exome'} @header;
      push(@header, 'spliceai_exome');
    }
    ## put annotations of spliceai_exome in multiple lines
    my (@multiplelines, @oneline);
		for my $i (0 .. @header-1) {
			my $item = $header[$i];
			my $expanded_field;
      ...
      if ($expanded_field){
        if (defined $varanno{$varstring}{$item}){
          my $varanno_text = $varanno{$varstring}{$item};
          if ( ($item eq 'spliceai_exome') and (ref($varanno_text) eq 'ARRAY')){
            for my $varanno_line(@$varanno_text){
              my @infos = map { $_ =~ s/\\x2c/,/g; $_ =~ s/\\x23/#/g; $_} split (/,/, $varanno_line); 
              my @newline = (@oneline, @infos );
              push @multiplelines, \@newline;
            }
          } else {
            my @infos = map { $_ =~ s/\\x2c/,/g; $_ =~ s/\\x23/#/g;} split (/,/, $varanno_text); 
            push @oneline, @infos;
          }

        }else{
          push @oneline, ($nastring) x $expanded_field;
        }
      }else{
					push @oneline, (defined $varanno{$varstring}{$item})? qq/"$varanno{$varstring}{$item}"/ : $nastring;	#value of $varanno{$varstring}{$item} could be zero
      }
		}
    # if spliceai_exome is not annotated, the content of multiple_lines and oneline is same
    if ( !grep (/spliceai_exome/, @header) ){
      push @multiplelines, \@oneline;
    }

    if ($csvout) {
      for my $oneline (@multiplelines){
        print OUT join (",", @varstring, @$oneline), $otherinfo?qq/,"$info"/:"", "\n";
      }
		} else {
      for my $oneline (@multiplelines){
...
sub geneOperation {
	...
	#generate gene anno
	my $sc;
	$sc = "annotate_variation.add_spliceai_exome.pl -geneanno -buildver $buildver -dbtype $protocol -outfile $tempfile.$protocol -exonsort -nofirstcodondel $queryfile $dbloc";		#20191010: add -nofirstcodondel
	...
	if ($polishgene) {
		if (rename ($e_anno_outfile, "$e_anno_outfile.orig")) {
			$codingarg ||= '';
			$sc = "coding_change.pl $codingarg $e_anno_outfile.orig $dbloc/${buildver}_$protocol.txt $dbloc/${buildver}_${protocol}Mrna.fa -alltranscript -out $tempfile.$protocol.fa -newevf $e_anno_outfile";
			print STDERR "\nNOTICE: Running with system command <$sc>\n";
			system ($sc) and die "Error running system command: <$sc>\n";
...
sub regionOperation {
	...
	my ($userfile, $header);
	my $sc = "annotate_variation.add_spliceai_exome.pl -regionanno -dbtype $dbtype -buildver $buildver -outfile $tempfile $queryfile $dbloc";
	$argument and $sc .= " $argument";
...
sub filterOperation {
	my ($dbtype, $dbtype1, $argument) = @_;
	my ($userfile, $header);
	my $sc = "annotate_variation.add_spliceai_exome.pl -filter -dbtype $dbtype -buildver $buildver -outfile $tempfile $queryfile $dbloc";
	$argument and $sc .= " $argument";
	if ($thread) {
		$sc .= " -thread $thread";
	}
    ...
    #MINSU
    if ($header=~/spliceai/){
      push @{ $varanno{$varstring}{$header} }, $anno;
    } else {
      $varanno{$varstring}{$header}=$anno;
    }
	}
